...
import { OpenClawManager, InstanceStatus, Instance } from '@openclaw/manager-core';
...
async function cloneCommand(args: string[]): Promise<void> {
  const id = args[0];
  ...
  switch (command) {
    ...
    case 'config': {
      ...
      if (id) {
        ...
        if (configPath) print(configPath);
